...
@noinline function read_matfrostarray_primitive!(io::BufferedStream, header::MATFrostArrayHeader, ::Type{T}) :: MATFrostArrayPrimitive{T}  where {T<:Number}
    values = Vector{T}(undef, header.nel)
    read!(io, values)
    MATFrostArrayPrimitive{T}(header.dims, values)
end

@noinline function read_matfrostarray_string!(io::BufferedStream, header::MATFrostArrayHeader) :: MATFrostArrayString
    values = String[read_string!(io) for _ in 1:header.nel]
    MATFrostArrayString(header.dims, values)
end

@noinline function read_matfrostarray_struct!(io::BufferedStream, header::MATFrostArrayHeader)::MATFrostArrayStruct
...
@noinline function read_matfrostarray_cell!(io::BufferedStream, header::MATFrostArrayHeader)::MATFrostArrayCell
    values = MATFrostArrayAbstract[
...
@noinline function read_matfrostarray!(io) :: MATFrostArrayAbstract
    header = read_matfrostarray_header!(io::BufferedStream)
